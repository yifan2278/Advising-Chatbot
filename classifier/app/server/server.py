...
if persister_implementation == "IN_MEMORY":
    ...
    persister = PicklePersister(os.path.join("./classifier/nlpc_data", "data"))
    model_persister = PicklePersister(os.path.join("./classifier/nlpc_data", "models"))
else:
    persister = FilePersister(os.path.join("./classifier/nlpc_data", "data"))
    model_persister = FilePersister(os.path.join("./classifier/nlpc_data", "models"))
...
def nlc_get(project_id, model_id: str):
    ...
    #################################
    import numpy as np
    print(model.clf.decision_function(features))
    print(np.max(model.clf.decision_function(features), axis=1))
    # if all value negative ==> no good pred
    #################################
